...
function App() {
  ...
  function newProject() {
    if (!confirm('Create a new empty project?')) {
      return;
    }
  ...
  function remove() {
    ...
    setFiles(newFiles);
    setTabs((currentTabs) =>
      currentTabs.filter((tab) => tab !== active),
    );
    setActive(firstFile);
    ...
    <div className={`app ${theme}`}>
      <header>
        ...
        <div className="actions">
          ...
          <button onClick={() => togglePanel('source')}>
            <GitBranch /> Source
          </button>
      ...
      <div className="workspace">
        ...
        <main>
          <div className="tabs">
            ...
            <button className="newtab" onClick={newFile}>
              <Plus />
            </button>
          </div>
          ...
          <div className="toolbar">
            ...
            <button
              onClick={() =>
                setZoom((value) => Math.min(24, value + 1))
              }
            >
              A+
            </button>
            ...
            <span>
              {draft === files[active]
                ? 'Saved'
                : 'Unsaved changes'}{' '}
              · {draft.length} chars · Ln {lines.length}
            </span>
          </div>
                    ...
                    'flux',
                    {
                      tokenizer: {
                        root: [
                          [/\/\/.*$/, 'comment'],
                          [
                            /\b(fn|function|let|const|if|else|return|true|false|struct|import|from|match|for|while|in)\b/,
                            'keyword',
                          ],
                          [/"[^"]*"/, 'string'],
                          [
      ...
      {palette && (
        ...
        >
          ...
          >
            ...
            <button onClick={() => command('project')}>
              <FolderPlus /> New Project
            </button>
            ...
            <button onClick={() => command('format')}>
              <Braces /> Format Document
            </button>

            <button onClick={() => command('split')}>
              <SplitSquareHorizontal /> Split Editor
            </button>

            <button onClick={() => command('terminal')}>
              <Terminal /> Terminal
            </button>

            <button onClick={() => command('problems')}>
              <CheckCircle2 /> Problems
            </button>

            <button onClick={() => command('outline')}>
              <Eye /> Toggle Outline
            </button>

            <button onClick={() => command('download')}>
              <Download /> Export
            </button>

            <button onClick={() => command('upload')}>
              <Upload /> Import
            </button>

            <button onClick={() => command('updates')}>
              <RefreshCw /> Check Updates
            </button>

            <button onClick={() => command('settings')}>
              <Settings /> Settings
            </button>
          </div>
        </div>
      ...
      {settings && (
        ...
        >
          ...
          >
            ...
            <p>
              Browser workspace data is stored locally. Native
              folders, terminal processes, debugging and real
              Git are reserved for the Electron/backend phase.
            </p>
