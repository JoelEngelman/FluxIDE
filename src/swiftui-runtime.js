...
function executeSwiftUI(source, filename) {
  ...
  var html = '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' + escapeHtml(title) + '</title><style>' +
    ...
    '</style></head><body><div class="device"><div class="status">FluxIDE SwiftUI Runtime · ' + escapeHtml(file) + '</div><div class="screen"><h2>' + escapeHtml(title) + '</h2>' + content + '</div></div></body></html>';
  ...
  return 'SwiftUI preview opened for ' + file + '. Rendered by FluxIDE — no Xcode, macOS, or Apple SDK required.';
}
